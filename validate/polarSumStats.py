...
def getOutputSummary(d, outputImgs=False, outputCoords=False):
    ...
    for psf_param, p_sum in zip([d['e1'], d['e2'], d_sigma], [e1, e2, size]):
        for pcf_size, radius, width, nbins in zip(['big', 'small'], [1.8, 0.05], [0.2, 0.05], [30, 18]):
...
if __name__ == '__main__':
    ...
    parser.add_argument("--outdir", type=str, default='/home/users/chebert/validate-psfws/summaries/allSixMonths_0407/')
    ...
    for num in range(538):
        ...
        size_sum[num], e1_sum[num], e2_sum[num], imgs, coords = getOutputSummary(d, outputCoords=True, outputImgs=True)
        atm_sum[num] = d['atmKwargs']

        images[num] = {k:imgs[i] for i,k in enumerate(['e1_big', 'e1_small', 'e2_big', 'e2_small', 'size_big', 'size_small'])}
    images['coord'] = coords    
